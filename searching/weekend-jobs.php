...
$schedule_type ="Evening & weekend jobs" ;

// Query to fetch job postings based on the selected schedule type
$sql = "SELECT * FROM job_posting WHERE `schedule_type` = '$schedule_type' AND status = 1"; // Only show active jobs
$stmt = $conn->prepare($sql);
$stmt->bind_param("s", $schedule_type); // Bind the schedule_type parameter
$stmt->execute();
